...
public class TbExchangeServiceImpl implements TbExchangeService {
    ...
    public void toGenerateExchangeOrderGoods(int[] orderId, int[] goodsId, int[] number) {
        ...
        for (int i=0;i<orderId.length;i++){
            ...
            tbReturnGoods1.setOrderId(orderId[i]);
...
//        TbReturn tbReturn=tbExchangeMapper.selectByPrimaryKey(id);
        ...
        for (int t:ids){
            ...
            if (1296000000<date.getTime()-tbOrderMapper.getOrderByOrderId(tbReturn.getOrderId()).get(0).getPaymentTime().getTime())
            {
    ...
    public int generateInput(int orderId) {
        ...
        for(TbReturnGoods t:tbReturnGoods){
            ...
            InRepertoryDTO inRepertoryDTO = new InRepertoryDTO(TOKENS,String.valueOf(orderId),
